...
/**
 * The hide flow reports its failures through `useHidePage`'s own state. Reading
 * that back off the hook object *after* awaiting `setHidden` returns the value
 * from the render the click handler was created in — always the previous
 * attempt's. These two cases pin the symptoms that produced: a failure with no
 * message at all, and a success carrying the last failure's message with it.
 */
describe('HistoryView: removing a page that the relay refuses', () => {
  ...
  it('does not carry the failed attempt into the next, successful one', async () => {
    ...
    expect(document.body.textContent).toContain('The page is removed')
  })
})

/**
 * Both flows on this page write to the same notice. A fixed headline meant the
 * hide confirmation was announced as a deletion — the one thing the whole
 * feature is built not to claim. docs/05-versioning-history.md
 */
describe('HistoryView: the success notice', () => {
  it('does not announce a hidden page as a deletion', async () => {
    vi.mocked(publishRevision).mockResolvedValue({ ok: true, message: 'ok' })
    await render()
