...
export type RevisionInput = {
  ...
  restoreOf?: string
  /**
   * Hide the page: the revision carries the tombstone tag and the page leaves
   * the tree, the search and the navigation. Publishing a later revision
   * without it brings the page back, so this is a step in the chain and not a
   * deletion. docs/05-versioning-history.md
   */
  tombstone?: boolean
}
...
export async function publishRevision(
  ...
  const tags: string[][] = [
    ...
    [
      TAGS.ALT,
      input.tombstone
        ? `Wiki page "${input.title}" was removed in space ${input.groupId}`
  ...
  if (input.tombstone) tags.push([TAGS.TOMBSTONE, '1'])
