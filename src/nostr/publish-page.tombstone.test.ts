...
describe('publishRevision with a tombstone', () => {
  afterEach(() => {
    vi.mocked(client.publish).mockReset()
  })

  it('writes the tag, and writes nothing when the page stays visible', async () => {
    expect((await publish({ ...base, tombstone: true })).tags).toContainEqual([TAGS.TOMBSTONE, '1'])

    const visible = await publish(base)
    expect(visible.tags.some((tag) => tag[0] === TAGS.TOMBSTONE)).toBe(false)
  })

  it('is still an ordinary revision — the text travels with it', async () => {
    const event = await publish({ ...base, tombstone: true })
  ...
  it('says in the alt text that the page was removed, for a foreign client', async () => {
    const event = await publish({ ...base, tombstone: true })
    const alt = event.tags.find((tag) => tag[0] === TAGS.ALT)![1]
    expect(alt).toContain('was removed')
  })

  it('round trips: published, parsed, and the page is out of the tree', async () => {
    const first = await publish(base)
    const removal = await publish({ ...base, tombstone: true, parentRevs: [first.id] })
